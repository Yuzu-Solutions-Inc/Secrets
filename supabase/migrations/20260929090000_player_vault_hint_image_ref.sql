-- Bank hint decks can carry an image hint that points at a bundled icon
-- (hints.image_ref, e.g. 'lucide:gift') instead of an uploaded asset. The
-- player vault reported has_image only from asset_path, so those hints
-- reached the client with no text and no image and rendered blank.
--
-- Whole function reproduced (Postgres has no "add one field"); only the
-- 'image_ref' key in the hints array is new, alongside the existing
-- 'has_image'.
