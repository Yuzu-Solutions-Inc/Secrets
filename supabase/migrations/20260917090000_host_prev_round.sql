-- Run-of-show header (item 22): the host can step the game back a round to
-- "move things around". prev_round re-opens the current round as 'scheduled'
-- and makes the previous round (by position) live again. Money is not
-- unwound — this is a sequencing tool, not an undo.
--
-- Whole function reproduced (Postgres has no "add one branch"); only the
-- `prev_round` elsif is new.
