-- Vault roster redesign.
--
-- The player screen is being cut down to four things (profile, money, the two
-- buzz buttons, the Vault). Everything else moves into the Vault, which becomes a
-- roster: one card per player with their photo, name, how many hints the caller
-- holds on them, and whether their secret is out. Tapping a card shows every hint
-- the caller holds, the secret text once revealed, the accusations the caller has
-- made, and a free-text comment that auto-saves. The House Secret is one more
-- card in the same roster and gets its own comment.
--
--  1. theory_notes: a second directed target so a note can point at the House
--     Secret instead of a player.
--  2. player_vault: extra per-player fields (avatar, revealed flag + text, hint
--     count), the caller's accusations, and a house block with its comment.

-- ---------------------------------------------------------------------------
-- 1. theory_notes.target_house_secret_id
-- ---------------------------------------------------------------------------
alter table public.theory_notes
  add column if not exists target_house_secret_id uuid
  references public.house_secrets(id) on delete cascade;

-- Non-partial, matching the existing per-player key. NULLs are distinct, so the
-- many general / per-player rows that leave this column NULL never collide.
create unique index if not exists theory_notes_game_player_house_key
  on public.theory_notes (game_id, player_id, target_house_secret_id);

-- ---------------------------------------------------------------------------
-- 2. player_vault read model
-- ---------------------------------------------------------------------------
