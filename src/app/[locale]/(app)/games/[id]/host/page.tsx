...
export default async function HostPage({
  ...
  const [
    ...
    supabase.from("secrets").select("*,secret_holders(player_id,game_players(id,user_id,profiles(display_name))),hints(id,kind,text,asset_path,position,default_price,released_at)").eq("game_id", id),
    ...
    supabase.from("ledger_transactions").select("id,type,description,created_at,idempotency_key,reversed_transaction_id,ledger_entries(amount,wallets(kind,player_id,game_players(profiles(display_name)),teams(name)))").eq("game_id", id).order("created_at", { ascending: false }).limit(50),
